<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:38:"application/admin\view\News\Index.html";i:1588920174;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\header.html";i:1588911510;s:74:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\left.html";i:1588817206;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\footer.html";i:1588130548;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="<?php echo $vs['listurl']; ?>"><?php echo $vs['catname']; ?></a></li>
			<li><a href="<?php echo $vs['addurl']; ?>"><?php echo $vs['catname']; ?>添加</a></li>
		</ul>
		<div class="layui-tab-content">
			<div class="layui-tab-item layui-show">
				<form action="" method="get" class="layui-form">
					<input type="hidden" name="pid" value="<?php echo $vs['pid']; ?>">
					<input type="hidden" name="ty" value="<?php echo $vs['ty']; ?>">
					<input type="hidden" name="tty" value="<?php echo $vs['tty']; ?>">
					<input type="hidden" name="ttty" value="<?php echo $vs['ttty']; ?>">
					<div class="layui-form-item nospace">
						...
						<div class="layui-input-inline">
							...
							<a class="layui-btn layui-btn-primary"  href="<?php echo $vs['listurl']; ?>">清除搜索</a>
						</div>
					</div>
				</form>

				<form action="<?php echo url('News/Action'); ?>" method="post" id="contentForm" name="formlist" class="layui-form" onkeydown="if(event.keyCode==13) return false;">
				<table class="layui-table">
					<thead>
					<tr>
						<th><input type="checkbox" class="checkbox" lay-ignore id="checkall" title="全选"> 全选</th>
						<th>序号|ID</th>
						<?php if($vs['imgnum'] > 0): ?><th>缩略图</th><?php endif; ?>
					...
					<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
						<tr>
							<td><input type="checkbox" class="checkbox checkitem" lay-ignore name="checkid[]" value="<?php echo $v['id']; ?>"></td>
							<td><?php echo $i; ?>|<?php echo $v['id']; ?></td>
							<?php if($vs['imgnum'] > 0): ?><td><img src="<?php echo (isset($v['img1']) && ($v['img1'] !== '')?$v['img1']:'/Uploads/nopic.jpg'); ?>" width="80" /></td><?php endif; ?>
							<td><?php echo $v['title']; if($v['isgood'] == 1): ?><font color="red">推荐</font><?php endif; if($v['istop'] == 1): ?><font color="blue">置顶</font><?php endif; ?></td>
							<td class="table-input">
 								<input type="text" name="sorting_<?php echo $v['id']; ?>" value="<?php echo $v['disorder']; ?>" class="layui-input">
							</td>
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo url('News/Audit',"id=".$v['id'].""); ?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo url('News/Audit',"id=".$v['id'].""); ?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							<td><?php echo get_time($v['sendtime'],'Y-m-d'); ?></td>
							<td>
								<a href="<?php echo url('News/Add',"pid=".$v['pid']."&ty=".$v['ty']."&id=".$v['id'].""); ?>"  class='layui-btn layui-btn-xs'>修改</a>
								<a href='<?php echo url('News/Del',"id=".$v['id'].""); ?>' onclick='return confirm("您确定要删除吗？\n\n删除后不可恢复!")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
							</td>
						</tr>
					<?php endforeach; endif; else: echo "" ;endif; ?>
					</tbody>
