<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:42:"application/admin\view\settings\index.html";i:1588760645;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\header.html";i:1588911510;s:74:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\left.html";i:1588817206;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\footer.html";i:1588130548;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    KindEditor.ready(function(K) {
		...
		<?php if(is_array($list6) || $list6 instanceof \think\Collection || $list6 instanceof \think\Paginator): $i = 0; $__LIST__ = $list6;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v6): $mod = ($i % 2 );++$i;?>
            K('#<?php echo $v6['varname']; ?>_btn').click(function() {
	...
	<div class="layui-body">
		<div class="layui-tab layui-tab-brief" lay-filter="tab">
			<ul class="layui-tab-title">
				...
				<?php if(\think\Session::get('admin_id') == 99999999): ?><li  lay-id="t5">添加新变量</li><?php endif; ?>
			</ul>
			<div class="layui-tab-content">
				<div class="layui-tab-item layui-show">
					<form action="<?php echo url('Settings/Update'); ?>" method="post" class="layui-form" name="myform1">
									...
									<?php elseif($v1['vartype'] == 3): ?>
										<div class="layui-input-inline">
											<select name="<?php echo $v1['varname']; ?>">
												...
												<?php
													$arr1=explode("|",$v1['varoption']);
													for($i=0;$i<count($arr1);$i++){
													$value1=$arr1[$i];
													if($v1['varvalue']==$value1) $s=" selected";else $s="";
													echo "<option value=\"$value1\"$s>$value1</option>";
													}
												?>
											</select>
										</div>
									<?php endif; ?>
								<font color="red"><?php echo $v1['varoption']; ?></font> <?php if(\think\Session::get('admin_id') == 99999999): ?>调用字段&nbsp;:sys('<?php echo $v1['varname']; ?>')<?php endif; ?>
							</div>
						<?php endforeach; endif; else: echo "" ;endif; ?>
						<div class="layui-form-item">
				...
				<div class="layui-tab-item">
					<form action="{my:U('Settings/Update')}" method="post" class="layui-form" name="myform2" >
						<?php if(is_array($list2) || $list2 instanceof \think\Collection || $list2 instanceof \think\Paginator): $i = 0; $__LIST__ = $list2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?>
							<div class="layui-form-item">
								<label class="layui-form-label"><?php echo $v1['varinfo']; ?></label>
									...
									<?php elseif($v1['vartype'] == 2): elseif($v1['vartype'] == 3): ?>
										<div class="layui-input-inline">
											<select name="<?php echo $v1['varname']; ?>">
												<option value="">请选择</option>
												<php>
				...
				<div class="layui-tab-item">
					<form action="<?php echo url('Settings/Update'); ?>" method="post" class="layui-form" name="myform3">
						<?php if(is_array($list3) || $list3 instanceof \think\Collection || $list3 instanceof \think\Paginator): $i = 0; $__LIST__ = $list3;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?>
							<div class="layui-form-item">
								<label class="layui-form-label"><?php echo $v1['varinfo']; ?></label>
									<?php if($v1['vartype'] == 0): ?>
									<div class="layui-input-inline">
									...
									<?php elseif($v1['vartype'] == 2): elseif($v1['vartype'] == 3): endif; ?>
								<font color="red"><?php echo $v1['varoption']; ?></font>
								<?php if($v1['varname']=='sys_ll'){?><a href="<?php echo url('Settings/Map'); ?>" style="color:blue" target="_blank">获取经纬度</a><?php }if(\think\Session::get('admin_id') == 99999999): ?> 调用字段&nbsp;:sys('<?php echo $v1['varname']; ?>')<?php endif; ?>
							</div>
						<?php endforeach; endif; else: echo "" ;endif; ?>
						<div class="layui-form-item">
							<div class="layui-input-block">
					...
					<div class="layui-tab-item">
						<form action="<?php echo url('Settings/Update'); ?>" method="post" class="layui-form" name="myform4">
							<?php if(is_array($list4) || $list4 instanceof \think\Collection || $list4 instanceof \think\Paginator): $i = 0; $__LIST__ = $list4;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?>
								<div class="layui-form-item">
									<label class="layui-form-label"><?php echo $v1['varinfo']; ?></label>
										<?php if($v1['vartype'] == 0): ?>
										<div class="layui-input-inline">
					...
					<div class="layui-tab-item">
					<form action="<?php echo url('Settings/Add'); ?>" method="post" class="layui-form" name="myform5" >
