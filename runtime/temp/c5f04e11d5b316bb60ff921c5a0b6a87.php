<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:38:"application/admin\view\sort\index.html";i:1591600937;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\header.html";i:1588911510;s:74:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\left.html";i:1588817206;s:76:"F:\phpStudy\PHPTutorial\WWW\think5\application\admin\view\Public\footer.html";i:1588130548;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="<?php echo url('Sort/Index'); ?>">栏目列表</a></li>
			<?php if(\think\Session::get('admin_id') == 99999999): ?>
				<li><a href="<?php echo url('Sort/Add'); ?>">栏目添加</a></li>
			<?php endif; ?>
		</ul>
				...
				<table class="layui-table">
					...
					<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
						<tr>
							...
							<td><?php echo str_repeat("&nbsp;&nbsp;└",$v['count']); ?><?php echo $v['catname']; ?></td>
							<td><?php echo get_model_zd($v['model_id']); ?></td>
							<td class="table-input">
								<input type="hidden" name="checkid[]" value="<?php echo $v['id']; ?>">
								<input type="text" name="sorting_<?php echo $v['id']; ?>" value="<?php echo $v['disorder']; ?>" class="layui-input"></td>
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo url('Sort/Audit',"id=".$v['id'].""); ?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo url('Sort/Audit',"id=".$v['id'].""); ?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							...
							<td>
								...
								<?php if($v['isopen']==1||session('admin_id')==99999999){?>
									<a href="<?php echo url('Sort/Add',"pid=".$v['id'].""); ?>" class='layui-btn layui-btn-xs'>添加子栏目</a>
									<a href='<?php echo get_url($v['id'],'Sort','Del'); ?>' onclick='return confirm("您确定要删除吗？")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
								<?php }?>
								<a href='<?php echo url('Sort/Add',"id=".$v['id'].""); ?>'  class='layui-btn layui-btn-xs layui-btn-danger'>修改</a>
							</td>
						</tr>
					<?php endforeach; endif; else: echo "" ;endif; ?>
					</tbody>
