	...
	$.ajax({
	  ...
	  success: function (response, status) {
			if (response.code == 1) {
				...
				layer.msg("登入失败：" + response.tip, {icon: 5});
			}
      },
  ...
  //选择模型切换模板
   form.on('select(model)', function(data){
	 ...
	 // addOptionValue("contenttpl",contenttpl,contenttpl);
	  $("#outlinkurl").val(listtpl);
	  $("#inlinkurl").val(contenttpl);
    ...
    form.on('select(pid)', function(data){
        ...
        $.get('action.php?act=gettype&pid='+v,null,function(data){
            $("#ty").html('<option value="0">选择二级</option>'+data);
            $("#tty").html('<option value=\"\">请选择</option>');
            form.render('select');
        });
    });
    ...
    form.on('select(ty)', function(data){
        ...
        var v = $(elem).find("option:selected").val();
        $.get('action.php?act=gettype&pid='+v,null,function(data){
            $("#tty").html('<option value="0">选择三级</option>'+data);
            form.render('select');
        });
    });
