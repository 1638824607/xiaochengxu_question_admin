...
class Backup extends Purview {
	...
	public function Import()
	{
		set_time_limit(0);
		$key=input('key');
		if($this->request->isGet()&&$key)
		{
			
			$key=str_replace('..','',$key);
			$name='public/Backup/'.$key;
			//echo $name;
			//exit();
			if(!is_file($name))
			{
	...
	public function Backup($table)
	{
		...
		foreach($list as $key)
		{
			...
			$data=Db()->query("SHOW CREATE TABLE `{$key}`");
			$sql .= "\n";
            $sql .= "-- -----------------------------\n";
		...
		file_put_contents('public/Backup/'.$name,$sql);
		echo '数据库备份成功';
	}
}
?>